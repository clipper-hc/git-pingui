...
const S = {
  temporada: { ...TEMPORADA },
  nedadors: [], banys: [], dies: {},
  config: { pontMin: 1400, pontMax: 1600, modeProves: true },
  criteri: "absoluta",   // absoluta | dies | metres
...
async function carregar() {
  ...
  S.banys = ban.data.map((b) => ({
    ...
    neopre: b.neopre, competitiu: b.competitiu, esmorzar: b.esmorzar,
    nota: b.nota, estat: b.estat,
  }));
  S.dies = Object.fromEntries(dia.data.map((d) =>
    [d.data, { tempC: d.temp_c === null ? null : Number(d.temp_c), doble: d.doble, font: d.font }]));
  if (cfg && cfg.data) {
    S.config = { pontMin: cfg.data.pont_min, pontMax: cfg.data.pont_max,
                 modeProves: cfg.data.mode_proves };
    S.temporada = { inici: cfg.data.inici, fi: cfg.data.fi };
  }
  reconeixerme();
}
...
const api = {
  ...
  async desarConfig(c, temporada) {
    S.config = c; S.temporada = temporada;
    if (!LIVE) return demoDesar();
    const { error } = await sb.from("config").upsert({
      id: 1, pont_min: c.pontMin, pont_max: c.pontMax,
      mode_proves: c.modeProves, inici: temporada.inici, fi: temporada.fi });
    if (error) throw new Error(error.message);
  },
  /* Només en mode de proves: esborra tots els banys per deixar-ho net abans
     de començar de debò. */
  async buidarBanys() {
    S.banys = [];
    if (!LIVE) return demoDesar();
    const { error } = await sb.from("banys").delete().neq("id",
...
/* ── Mode demostració ──────────────────────────────────────────────────── */
const DEMO_KEY = "copa-pingui-demo-v3";
function demoDesar() {
  try { localStorage.setItem(DEMO_KEY, JSON.stringify({
    nedadors: S.nedadors, banys: S.banys, dies: S.dies,
    config: S.config, temporada: S.temporada, joId: S.jo && S.jo.id })); } catch (e) { /* sense desar */ }
}
...
function demoSembrar() {
  ...
  S.nedadors.forEach((n, i) => {
    const neoprener = i === 3 || i === 6;            // dos que gairebé sempre en porten
    const ganes = neoprener ? 0.85 : 0.08 + rnd() * 0.55;
    const fons  = (neoprener ? 1800 : 600) + Math.floor(rnd() * 1900);
    dies.forEach((iso) => {
      ...
      S.banys.push({
        ...
        neopre: rnd() < (neoprener ? 0.92 : 0.05), competitiu: rnd() < 0.03,
  ...
  S.banys.push(
    { id: "r1", nedadorId: "n4", data: "2026-03-29", metres: 1500, neopre: false,
      competitiu: false, esmorzar: true, nota: "Fins al pont i tornar", estat: "aprovat" },
    { id: "r2", nedadorId: "n7", data: "2026-03-30", metres: 2400, neopre: false,
      competitiu: false, esmorzar: false, nota: "", estat: "aprovat" });
...
function classificacio() {
  return calcularClassificacio({
    ...
    temporada: S.temporada, config: S.config,
  });
}
...
const CRITERI_INFO = {
  absoluta: { valor: (r) => fmtPunts(r.total), unitat: "punts",
    nota: "Toca un nedador per veure mes a mes quins dies li compten i quins li han quedat fora pel límit de quatre." },
  dies: { valor: (r) => r.diesNedats, unitat: "dies",
    nota: "Dies diferents amb bany registrat dins de temporada. Compta tot: també amb neoprè i també les proves competitives." },
  metres: { valor: (r) => (r.metresNedats / 1000).toLocaleString("ca-ES", { maximumFractionDigits: 1 }), unitat: "km",
    nota: "Metres sumats dins de temporada. Compta tot: també amb neoprè i també les proves competitives." },
};

function pintarClassificacio() {
  const res = ordenar(classificacio(), S.criteri);
  const info = CRITERI_INFO[S.criteri];
  const board = $("#board");
  const ambPunts = res.filter((r) => r.total > 0).length;
  $("#boardMeta").textContent = `${res.length} inscrits · ${ambPunts} amb punts`;
  $("#boardNota").textContent = info.nota;
  $$(".filtre").forEach((f) =>
    f.setAttribute("aria-selected", String(f.dataset.crit === S.criteri)));
  ...
  board.innerHTML = res.map((r) => {
    ...
    const puntua = S.criteri === "absoluta" ? r.total > 0 : info.valor(r) !== 0;
    ...
    return `<li>
      <button class="entry${r.posicio === 1 && puntua ? " lead" : ""}${jo ? " jo" : ""}"
              data-ned="${esc(r.nedador.id)}" aria-expanded="${obert}">
        <span class="pos num">${puntua ? r.posicio : "—"}</span>
        <span class="who">
          <span class="nom">${esc(r.nedador.nom)}${jo ? ' <span class="tu">tu</span>' : ""}</span>
          <span class="sub">${S.criteri === "absoluta"
            ? `${r.diesNedats} dies · ${(r.metresNedats / 1000).toLocaleString("ca-ES",{maximumFractionDigits:1})} km`
            : `${fmtPunts(r.total)} punts a l'absoluta`}${
            r.tempMinima !== null ? ` · mínima ${fmtTemp(r.tempMinima)}` : ""}${
            r.elegible ? "" : ` · ${r.esmorzars}/2 esmorzars`}</span>
        </span>
        <span class="pts num">${info.valor(r)}<small>${info.unitat}</small></span>
...
function pintarElsMeus() {
  ...
  box.innerHTML = `<table class="tbl">
    <thead><tr><th>Dia</th><th>Metres</th><th>Aigua</th><th>Esm.</th><th>Punts</th><th></th></tr></thead>
    <tbody>${meus.slice(0, 20).map((b) => {
      ...
      return `<tr>
        ...
        <td class="n" style="color:${b.esmorzar ? "var(--good)" : "var(--muted)"}">${
          b.esmorzar ? "sí" : "—"}</td>
...
function pintarCoordinacio() {
  ...
  // Esmorzars: només el recompte, perquè ara els reporta cada nedador.
  const res = classificacio();
  const ambEsmorzars = [...res].sort((a, b) => b.esmorzars - a.esmorzars ||
    a.nedador.nom.localeCompare(b.nedador.nom, "ca"));
  $("#bList").innerHTML = `<table class="tbl">
    <thead><tr><th>Nedador</th><th>Esmorzars</th><th>Premi</th></tr></thead>
    <tbody>${ambEsmorzars.map((r) => `<tr>
      <td>${esc(r.nedador.nom)}</td>
      <td class="n">${r.esmorzars}</td>
      <td>${r.elegible ? '<span class="pill ok">Hi opta</span>'
        : `<span class="pill warn">Li'n falta${2 - r.esmorzars > 1 ? "n" : ""} ${2 - r.esmorzars}</span>`}</td>
    </tr>`).join("")}</tbody></table>`;
  ...
  $("#cInici").value = S.temporada.inici;
  $("#cFi").value = S.temporada.fi;
  $("#cProves").checked = S.config.modeProves === true;
  $("#cBuidar").classList.toggle("hidden", !S.config.modeProves);
}
...
function pintarFranja() {
  const barra = $("#demoBar");
  if (!LIVE) return;   // en demostració la franja ja diu una altra cosa
  const proves = S.config.modeProves === true;
  barra.classList.toggle("hidden", !proves);
  if (proves) barra.textContent =
    "Mode de proves · les dades d'ara no valen per a la copa · els coordinadors "
    + "el poden desactivar des de Coordinació";
}
...
function aplicarBloqueig() {
  ...
  $("#coordLockedMsg").textContent = S.usuari
    ? "Aquesta secció és per a Hugo i Jordi: fixar temperatures que l'AEMET no doni, marcar el dia sorpresa, les dates de la temporada i corregir errors."
...
function connectar() {
  ...
  $("#board").parentElement.querySelector(".filtres")
    .addEventListener("click", (e) => {
      const f = e.target.closest("[data-crit]"); if (!f) return;
      S.criteri = f.dataset.crit; S.obert = null; pintarClassificacio();
    });
  ...
  $("#logForm").addEventListener("submit", async (e) => {
    ...
    if (!S.config.modeProves &&
        (bany.data < S.temporada.inici || bany.data > S.temporada.fi))
  ...
  $("#cSave").addEventListener("click", async () => {
    ...
    const inici = $("#cInici").value, fi = $("#cFi").value;
    if (!(min > 0 && max >= min))
      return missatge("#cMsg", "La banda del pont no és vàlida.", "err");
    if (!inici || !fi || fi <= inici)
      return missatge("#cMsg", "El final de la temporada ha de ser posterior a l'inici.", "err");
    try {
      await api.desarConfig({ pontMin: min, pontMax: max, modeProves: $("#cProves").checked },
        { inici, fi });
      missatge("#cMsg", "Configuració desada.");
      pintar();
    } catch (err) { missatge("#cMsg", "No s'ha pogut desar: " + err.message, "err"); }
  });

  $("#cBuidar").addEventListener("click", async () => {
    if (!S.config.modeProves) return;
    if (!confirm(`Això esborrarà els ${S.banys.length} banys registrats, de tothom `
      + "i sense possibilitat de recuperar-los. Segur?")) return;
    try { await api.buidarBanys(); missatge("#cMsg", "Banys esborrats."); pintar(); }
    catch (err) { missatge("#cMsg", "No s'ha pogut buidar: " + err.message, "err"); }
  });
}
