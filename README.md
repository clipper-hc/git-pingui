...
test-scoring.js 46 proves del motor      ·  node test-scoring.js
...
Els correus gratuïts de Supabase van limitats (unes poques desenes al dia).
Per a 20 persones que entren un cop i es queden identificades, de sobres.

### 5. Google

1. A [Google Cloud Console](https://console.cloud.google.com/), crea un
   projecte i ves a **APIs & Services → OAuth consent screen**. Tipus
   **External**, publica'l, i afegeix-hi el teu correu com a contacte.
2. **Credentials → Create credentials → OAuth client ID → Web application**.
3. A *Authorized redirect URIs*, enganxa exactament la que et dona Supabase a
   **Authentication → Providers → Google**. Té aquesta forma:
   `https://xxxxxxxx.supabase.co/auth/v1/callback`.
4. Copia el *Client ID* i el *Client secret* i enganxa'ls a Supabase, al
   proveïdor Google. Activa'l.

Mentre la pantalla de consentiment estigui en mode *Testing*, només hi poden
entrar els correus que hagis afegit a *Test users*. Publicant-la, hi pot
entrar qualsevol. Per a una copa de 20 amics, publicar-la és més còmode que
anar afegint correus.
...
| | Mirar la classificació | Apuntar banys | Esborrar els seus | Temperatures, dobles, esmorzars |
|---|---|---|---|---|
...
Tot `scoring.js`, amb 46 proves a `test-scoring.js`:
...
## Canviar de temporada

Al novembre següent, actualitza `TEMPORADA` a `index.html`. Les dades velles
es queden a la base de dades i deixen de comptar soles, perquè el motor
descarta el que cau fora del període.
