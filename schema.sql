...
create table if not exists esmorzars (
  id         uuid primary key default gen_random_uuid(),
  data       date not null,
  assistents jsonb not null default '[]'::jsonb
);
...
drop policy if exists llegir_esmorzars on esmorzars;
...
create policy llegir_esmorzars on esmorzars for select using (true);
...
-- Els coordinadors: temperatures, dies dobles, esmorzars, la banda del pont,
-- i anul·lar un bany equivocat. El reglament ja els donava l'última paraula.
...
drop policy if exists coord_esmorzars on esmorzars;
...
create policy coord_esmorzars on esmorzars for all to authenticated using (soc_coordinador()) with check (soc_coordinador());
