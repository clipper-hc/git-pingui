...
/* ==========================================================================
   ...
   banys      : [{ id, nedadorId, data:"YYYY-MM-DD", metres, neopre, competitiu,
                   estat:"pendent"|"aprovat"|"rebutjat", nota }]
   dies       : { "YYYY-MM-DD": { tempC, doble } }
   nedadors   : [{ id, nom }]
   esmorzars  : [{ data, assistents:[nedadorId] }]
   ========================================================================== */
function calcularClassificacio({ banys, dies = {}, nedadors, esmorzars = [], temporada, config = {} }) {
  ...
  for (const n of nedadors) perNedador.set(n.id, { nedador: n, dies: new Map(), descartats: [] });
  ...
  for (const fitxa of perNedador.values()) {
    ...
    const esmorzarsAssistits = esmorzars.filter((e) =>
      (e.assistents || []).includes(fitxa.nedador.id)
    ).length;
  ...
  // 3. Ordena i assigna posicions (empats comparteixen posició).
  resultats.sort((a, b) => b.total - a.total || b.diesTotals - a.diesTotals ||
    a.nedador.nom.localeCompare(b.nedador.nom, "ca"));
  let posicio = 0, anterior = null;
  resultats.forEach((r, i) => {
    if (r.total !== anterior) { posicio = i + 1; anterior = r.total; }
    r.posicio = posicio;
  });
...
if (typeof module !== "undefined") {
  module.exports = { calcularClassificacio, puntsTemperatura, coeficientDistancia, esDiaDoble, DEFAULTS };
}
